...
const SalesReportTable = ({
  ...
  const doSearch = (e) => {
    e.preventDefault()
    let params = {date_type:"payment_date", status: "1,2,3"}
    if( searchKeyword ){
  ...
  const doClearFilter = (e) => {
    ...
    setAllOrder(dispatch, 0, {date_type:"payment_date", status: "1,2,3"})
  }

  const handlePageChange = (pageNumber) => {
    setActivePage(pageNumber)
    setAllOrder(dispatch, (pageNumber-1)*10, {date_type:"payment_date", status: "1,2,3"})
  }
