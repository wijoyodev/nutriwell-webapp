...
const FieldHandler = ({
  ...
  } else if (item.type === "labelWithValue"){
    return (
      <Col md={item.spaceMd} xs={item.spaceXs} key={index} className={styles.section}>
        ...
        {
          item.withCurrency ? 
            <Form.Label htmlFor="basic-url" className={styles.label_bold}>{'Rp. ' + item.value}</Form.Label> 
          :
            <Form.Label htmlFor="basic-url" className={styles.label_bold}>{item.value}</Form.Label> 
        }
      </Col>
    )
  } else if (item.type === "labelTitle"){
    return (
      <Col md={item.spaceMd} xs={item.spaceXs} key={index} className={styles.section}>
        <Form.Label htmlFor="basic-url" className={styles.main_title_2}>{item.label}</Form.Label> 
      </Col>
    )
