...
export const setIslandResp = async (dispatch) => {
  axios.get(`${process.env.REACT_APP_API_URL}/v1/location/island`, {
  }).then(({data}) => {
    dispatch({ type: 'SET_ISLAND_RESP', payload: data })
  }).catch((error)=>{
...
export const setDetailShipyard = async (dispatch, id) => {
  // axios.get(`${process.env.REACT_APP_API_URL}/v1/shipyard-owner/${id}`,{
  //   headers: {
  //     "Authorization": localStorage.getItem('token'),
  //     "Cache-Control": "no-cache",
  //     "Content-Type": "application/json",
...
export const setSearchShipyardOwner = async (dispatch, params) => {
  const options = {
    method  : 'get',
    headers : {
      'Authorization': localStorage.getItem('token'),
    ...
    params  : params,
    url     : `${process.env.REACT_APP_API_URL}/v1/shipyard-owner`,
  }
